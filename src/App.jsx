...
// Full application: Screening → Calculator → Plan → Vault → Stress Test
// ─────────────────────────────────────────────────────────────────────────────
...
function ScreeningSection({ onComplete, lang }) {
  ...
  const required = { 1: ["maritalStatus","livingSituation","bpjsActive"], 2: ["incomeBracket","husbandEmployment","wifeEmployment"], 3: ["primaryGoal"], 4: ["riskTolerance"] };
  const canNext = (required[step] ?? []).every(f => {
    const v = profile[f]; return Array.isArray(v) ? v.length > 0 : v !== null && v !== undefined;
  });

  const babyInGoals = profile.primaryGoal === "baby" || profile.supportingGoals.includes("baby");

  const incomeBracketChips = Object.entries(INCOME_BRACKETS).map(([k, v]) => ({ value: k, label: v.label[lang] }));
  const employmentChips = (extra = false) => [
    ...
    ...(extra ? [{ value: "not_working", label: lang === "id" ? "Tidak Bekerja" : "Not Working" }] : []),
  ];

  const goalChips = Object.values(GOAL_DEFINITIONS).map(g => ({
    value: g.id, label: g.label[lang], sublabel: g.sublabel[lang],
    disabled: profile.supportingGoals.includes(g.id) && profile.primaryGoal !== g.id,
  }));
      ...
      <Stack gap={16}>
        ...
        {profile.maritalStatus && profile.maritalStatus !== "married" && (
          <div>
            ...
            <SliderRow label={lang === "id" ? "Estimasi bulan lagi" : "Estimated months away"} min={1} max={36} step={1} value={profile.weddingMonthsAway ?? 6} onChange={v => set("weddingMonthsAway", v)} display={`${profile.weddingMonthsAway ?? 6} ${lang === "id" ? "bulan" : "months"}`} />
          </div>
        )}
        ...
        <div>
          ...
          {profile.existingDebt && (
            <div style={{ marginTop: 10 }}>
              ...
              <ChipGroup chips={Object.entries(COPY.screening.q_existingDebtAmount.options).map(([k,v]) => ({ value: k, label: v[lang] }))} selected={profile.existingDebtAmount} onSelect={v => set("existingDebtAmount", v)} />
            </div>
          )}
        </div>
      </Stack>
      ...
      <Stack gap={16}>
        ...
        <div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: T.textHint, marginBottom: 6 }}>
            <span>{t("screening.q_lifestyleVsSpeed.left", lang)}</span>
            <span>{t("screening.q_lifestyleVsSpeed.right", lang)}</span>
          </div>
          <input type="range" min={1} max={5} step={1} value={profile.lifestyleVsSpeed} onChange={e => set("lifestyleVsSpeed", Number(e.target.value))} style={{ width: "100%", accentColor: T.green400 }} />
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: T.textHint, marginTop: 4 }}>
            {[1,2,3,4,5].map(n => <span key={n} style={{ fontWeight: profile.lifestyleVsSpeed === n ? 500 : 400, color: profile.lifestyleVsSpeed === n ? T.green600 : T.textHint }}>{n}</span>)}
          </div>
        </div>
...
// CALCULATOR SECTION
// ─────────────────────────────────────────────────────────────────────────────
function CalculatorSection({ scenario, onUpdate, goals, year, lang, computed }) {
  ...
  const nutritionChips = [
    ...
    { value: 400_000, label: lang === "id" ? "ASI + Formula (Rp400rb)" : "Mixed (Rp400K)" },
    { value: 800_000, label: lang === "id" ? "Formula Penuh (Rp800rb)" : "Full Formula (Rp800K)" },
  ];
      ...
      <Grid2 gap={12}>
        ...
        <Stack gap={0}>
          ...
          <Card style={{ marginBottom: 12 }}>
            ...
            {scenario.rentActive && <SliderRow label={lang === "id" ? "Jumlah sewa / KPR" : "Rent / mortgage amount"} min={5e5} max={12e6} step={25e4} value={scenario.rentAmount ?? 0} onChange={v => onUpdate({ rentAmount: v })} display={fmt(scenario.rentAmount ?? 0)} style={{ marginTop: 10 }} />}
            <SliderRow label={lang === "id" ? "IPL / maintenance" : "IPL / maintenance"} min={0} max={3e6} step={1e5} value={scenario.ipl ?? 0} onChange={v => onUpdate({ ipl: v })} display={fmt(scenario.ipl ?? 0)} />
            <SliderRow label={lang === "id" ? "Sinking fund properti" : "Property sinking fund"} min={0} max={1e6} step={5e4} value={scenario.sinkingFund ?? 0} onChange={v => onUpdate({ sinkingFund: v })} display={fmt(scenario.sinkingFund ?? 0)} hint={lang === "id" ? "Rp50–100rb/m²/tahun (standar Astra Land)" : "Rp50–100K/m²/year (Astra Land standard)"} />
          </Card>
          ...
          <Card style={{ marginBottom: 12 }}>
            ...
            {scenario.carCicilanActive && <SliderRow label={lang === "id" ? "Cicilan mobil / bulan" : "Car instalment / month"} min={5e5} max={8e6} step={25e4} value={scenario.carCicilanAmount ?? 0} onChange={v => onUpdate({ carCicilanAmount: v })} display={fmt(scenario.carCicilanAmount ?? 0)} style={{ marginTop: 10 }} />}
            <SectionLabel style={{ marginTop: 10 }}>{lang === "id" ? "Moda harian" : "Daily transport mode"}</SectionLabel>
          ...
          <Card style={{ marginBottom: 12 }}>
            <SliderRow label={lang === "id" ? "Dana darurat saat ini" : "Current emergency fund"} min={0} max={12} step={1} value={scenario.emergencyFundMonths ?? 0} onChange={v => onUpdate({ emergencyFundMonths: v })} display={`${scenario.emergencyFundMonths ?? 0} ${lang === "id" ? "bulan" : "mo"}`} />
            <SliderRow label={lang === "id" ? "Target dana darurat" : "Emergency fund target"} min={3} max={12} step={1} value={scenario.emergencyFundTarget ?? 6} onChange={v => onUpdate({ emergencyFundTarget: v })} display={`${scenario.emergencyFundTarget ?? 6} ${lang === "id" ? "bulan" : "mo"}`} />
          </Card>
        </Stack>
        ...
        <Stack gap={0}>
          ...
          <Card style={{ marginBottom: 12 }}>
            ...
            <SliderRow label={lang === "id" ? "BPJS Kesehatan" : "BPJS Health Insurance"} min={1e5} max={6e5} step={5e4} value={scenario.bpjsPremium ?? 0} onChange={v => onUpdate({ bpjsPremium: v })} display={fmt(scenario.bpjsPremium ?? 0)} />
            <SliderRow label={lang === "id" ? "Sosial & gaya hidup" : "Social & lifestyle"} min={2e5} max={6e6} step={1e5} value={scenario.lifestyleSpend ?? 0} onChange={v => onUpdate({ lifestyleSpend: v })} display={fmt(scenario.lifestyleSpend ?? 0)} />
            <SliderRow label={lang === "id" ? "Cicilan / utang lain" : "Debt / loan payments"} min={0} max={8e6} step={1e5} value={scenario.debtMonthlyPayment ?? 0} onChange={v => onUpdate({ debtMonthlyPayment: v })} display={fmt(scenario.debtMonthlyPayment ?? 0)} />
          </Card>

          <SectionLabel>{lang === "id" ? "Investasi & Tabungan" : "Investment & Savings"}</SectionLabel>
          <Card style={{ marginBottom: 12 }}>
            <SliderRow label={lang === "id" ? "Target investasi bulanan" : "Monthly investment target"} min={0} max={12e6} step={25e4} value={scenario.monthlyInvestment ?? 0} onChange={v => onUpdate({ monthlyInvestment: v })} display={fmt(scenario.monthlyInvestment ?? 0)} hint={lang === "id" ? `Target ideal: ${fmtP(THRESHOLDS.investmentTargetPct * 100)} dari income` : `Ideal target: ${fmtP(THRESHOLDS.investmentTargetPct * 100)} of income`} />
            {cashflow && (
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: T.textHint, marginBottom: 4 }}>
                  ...
                  <span style={{ color: cashflow.investPct >= 20 ? T.green600 : cashflow.investPct >= 13 ? T.amber600 : T.red600, fontWeight: 500 }}>{fmtP(cashflow.investPct)}</span>
                </div>
          ...
          <SectionLabel>{lang === "id" ? "Rencana Keluarga (Bayi)" : "Family Planning (Baby)"}</SectionLabel>
          <Card style={{ marginBottom: 12 }}>
            <Toggle label={lang === "id" ? "Ada rencana punya anak" : "Planning to have a baby"} on={scenario.babyPlanned} onToggle={() => onUpdate({ babyPlanned: !scenario.babyPlanned })} />
            {scenario.babyPlanned && (
              <Stack gap={12} style={{ marginTop: 14 }}>
                <div>
                  ...
                  <ConceptionTimeline value={scenario.conceptionMonthsFromNow === 0 ? "now" : scenario.conceptionMonthsFromNow} onChange={v => onUpdate({ conceptionMonthsFromNow: v === "now" ? 0 : v === "not_sure" ? 18 : Number(v) })} lang={lang} />
                </div>
                <div>
                ...
                <SliderRow label={lang === "id" ? "Vaksin privat & dokter anak" : "Private vaccines & pediatrics"} min={0} max={25e5} step={1e5} value={scenario.vaccinationPeds ?? 0} onChange={v => onUpdate({ vaccinationPeds: v })} display={fmt(scenario.vaccinationPeds ?? 0)} />
                <Toggle label={lang === "id" ? "Pengasuhan kakek/nenek (hemat ~Rp2,95 jt/bln)" : "Grandparent childcare (saves ~Rp2.95M/mo)"} on={(scenario.grandparentCareOffset ?? 0) > 0} onToggle={() => onUpdate({ grandparentCareOffset: (scenario.grandparentCareOffset ?? 0) > 0 ? 0 : 2_950_000 })} />
              </Stack>
            )}
          </Card>
        </Stack>
      ...
      {cashflow && (
        <>
          ...
          <Card style={{ padding: "16px 18px", marginBottom: 12 }}>
            ...
            <FlowRow label={lang === "id" ? "IPL + sinking fund" : "IPL + sinking fund"} value={`–${fmt(cashflow.iplCost + cashflow.sinkCost)}`} hidden={cashflow.iplCost + cashflow.sinkCost === 0} />
            ...
            <FlowRow hidden={!scenario.babyPlanned} label={lang === "id" ? "Baby overhead" : "Baby overhead"} value={`–${fmt(cashflow.babyCost)}`} />
          ...
          {verdict && (
            <VerdictBox
              verdict={verdict}
              title={t(`verdict.${verdict.level}.title`, lang)}
              body={t(`verdict.${verdict.level}.body`, lang, { gap: fmt(verdict.gap) })}
          ...
          <Grid2 gap={12}>
            ...
            <div>
              ...
              <Grid2 gap={8}>
                <MetricCard label={lang === "id" ? "Tabungan/tahun" : "Savings/year"} value={fmt(computed?.monthsToEF != null ? Math.max(0, cashflow.monthlyMargin + cashflow.investmentAmount) * 12 : 0)} sub={lang === "id" ? "estimasi" : "estimated"} />
                ...
                <MetricCard label={lang === "id" ? "% investasi" : "% invested"} value={fmtP(cashflow.investPct)} sub={lang === "id" ? "Target: 26%" : "Target: 26%"} accent={cashflow.investPct >= 20 ? T.green800 : cashflow.investPct >= 13 ? T.amber800 : T.red800} />
              </Grid2>
            </div>
          </Grid2>
        </>
...
// PLAN SECTION
// ─────────────────────────────────────────────────────────────────────────────
function PlanSection({ computed, scenario, activeLevers, onToggleLever, onApplyLevers, goals, lang }) {
    ...
    <Stack gap={16}>
      ...
      {levers?.length > 0 && (
        <div>
          ...
          {/* Adjusted result after active levers */}
          {activeLevers.length > 0 && adjusted && (
            <Card style={{ marginTop: 12, border: `1.5px solid ${T.green400}`, background: T.green50 }}>
              ...
              <Row justify="space-between" style={{ marginBottom: 12 }}>
                <Body size={13}>{lang === "id" ? "Margin" : "Margin"}</Body>
                <Body size={13} style={{ fontWeight: 500 }}>{fmtP(adjusted.adjustedCashflow.marginPct)}</Body>
              </Row>
              ...
              return (
                <Card key={gr.id}>
                  ...
                  {gr.monthsToReach > 0 && gr.monthsToReach < 99 && (
                    <Hint>{lang === "id" ? `Estimasi tercapai dalam ${gr.monthsToReach} bulan` : `Estimated ${gr.monthsToReach} months to reach`}</Hint>
                  )}
                </Card>
              );
            })}
...
function VaultSection({ vault, onSave, onLoad, onClear, scenario, computed, lang }) {
  ...
  const slots = Object.values(vault.slots);
    ...
    <Stack gap={16}>
      ...
      <Grid3 gap={10}>
        {slots.map(slot => {
          ...
          const er      = slot.engineResult;
          const cf      = er?.cashflow;
          const vd      = er?.verdict;
          return (
            <Card key={slot.id} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <Row justify="space-between" align="flex-start">
                ...
                {hasSave && <Badge color={vd?.level === "optimal" || vd?.level === "go" ? "green" : vd?.level === "conditional" ? "amber" : "red"}>{vd?.level?.toUpperCase() ?? "—"}</Badge>}
              </Row>

              {hasSave && cf && (
                <Stack gap={4}>
                  <Row justify="space-between">
                    <Hint>{lang === "id" ? "Income" : "Income"}</Hint>
                    <Hint style={{ fontWeight: 500, color: T.text }}>{fmt(cf.totalIncome)}</Hint>
                  </Row>
                  <Row justify="space-between">
                    <Hint>{lang === "id" ? "Sisa kas" : "Free cash"}</Hint>
                    <Hint style={{ fontWeight: 500, color: cf.monthlyMargin >= 0 ? T.green800 : T.red800 }}>{fmt(cf.monthlyMargin)}</Hint>
                  </Row>
                  <Row justify="space-between">
                    <Hint>{lang === "id" ? "Margin" : "Margin"}</Hint>
                    <Hint style={{ fontWeight: 500 }}>{fmtP(cf.marginPct)}</Hint>
                  </Row>
              ...
              {!hasSave && (
                <div style={{ textAlign: "center", padding: "12px 0", color: T.textHint, fontSize: 12 }}>
                  {t("vault.empty", lang)}
                </div>
              )}
              ...
              <Stack gap={6}>
                {scenario && (
                  confirmSlot === slot.id
                    ? <Row gap={6}>
                        <Btn size="sm" variant="primary" onClick={() => { onSave(slot.id); setConfirmSlot(null); }} style={{ flex: 1 }}>
                          {lang === "id" ? "Ya, timpa" : "Yes, overwrite"}
                        </Btn>
                        <Btn size="sm" variant="outline" onClick={() => setConfirmSlot(null)} style={{ flex: 1 }}>
      ...
      {/* Comparison */}
      {slots.filter(s => s.scenario && s.engineResult).length >= 2 && (
        <div>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              ...
              <tbody>
                {[
                  { key: "totalIncome",   label: lang === "id" ? "Total Income" : "Total Income",   fmt: fmt },
                  { key: "totalExpenses", label: lang === "id" ? "Pengeluaran"   : "Expenses",       fmt: fmt },
                  { key: "monthlyMargin", label: lang === "id" ? "Sisa Kas"     : "Free Cash",       fmt: fmt },
                  { key: "marginPct",     label: lang === "id" ? "Margin %"     : "Margin %",        fmt: fmtP },
                  { key: "investPct",     label: lang === "id" ? "Investasi %"  : "Investment %",    fmt: fmtP },
                ].map(row => (
                  <tr key={row.key}>
  ...
  return (
    <Stack gap={16}>
      ...
      <Card>
        ...
        <SliderRow label={lang === "id" ? "Total durasi cuti melahirkan" : "Total maternity leave duration"} min={1} max={12} step={1} value={leaveDuration} onChange={setLeaveDuration} display={`${leaveDuration} ${lang === "id" ? "bulan" : "months"}`} />
        <SliderRow label={lang === "id" ? "Berapa bulan dibayar penuh?" : "How many months are fully paid?"} min={0} max={leaveDuration} step={1} value={Math.min(paidMonths, leaveDuration)} onChange={v => setPaidMonths(Math.min(v, leaveDuration))} display={`${Math.min(paidMonths, leaveDuration)} ${lang === "id" ? "bulan" : "months"}`} hint={lang === "id" ? "Minimum 3 bulan sesuai UU Ketenagakerjaan" : "Minimum 3 months per Indonesian labour law"} />
      </Card>

      {result && (
        <>
          <Grid4 gap={8}>
            <MetricCard label={lang === "id" ? "Status dana darurat" : "Emergency fund status"} value={result.efSurvives ? (lang === "id" ? "Cukup" : "Sufficient") : (lang === "id" ? "Tidak Cukup" : "Insufficient")} accent={result.efSurvives ? T.green800 : T.red800} />
            <MetricCard label={lang === "id" ? "Total defisit kumulatif" : "Total cumulative deficit"} value={result.totalDeficit > 0 ? fmt(result.totalDeficit) : (lang === "id" ? "Nihil" : "None")} accent={result.totalDeficit > 0 ? T.red800 : T.green800} />
          ...
          <Card>
            ...
            <Row gap={12} style={{ marginTop: 8 }}>
              {[["ok", T.green400, lang === "id" ? "Aman" : "OK"], ["warning", T.amber400, lang === "id" ? "Perhatian" : "Warning"], ["critical", T.red400, lang === "id" ? "Kritis" : "Critical"]].map(([k, c, l]) => (
                <Row key={k} gap={4}>
                  <div style={{ width: 8, height: 8, background: c, borderRadius: 2, marginTop: 1 }} />
                  <Hint>{l}</Hint>
                </Row>
              ))}
            </Row>
          ...
          <Card>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                <thead>
                  <tr>
                    {[lang === "id" ? "Bulan" : "Month", lang === "id" ? "Status" : "Status", lang === "id" ? "Income" : "Income", lang === "id" ? "Pengeluaran" : "Expenses", lang === "id" ? "Net" : "Net", "EF"].map(h => (
                ...
                <tbody>
                  {result.monthByMonth.slice(0, leaveDuration + 3).map(m => (
                    <tr key={m.month}>
                      <td style={{ padding: "5px 8px", textAlign: "right", borderBottom: `0.5px solid ${T.border}`, color: m.onLeave ? T.amber800 : T.textMuted }}>
                        M{m.month}{m.onLeave ? "*" : ""}
                      </td>
                      ...
                      <td style={{ padding: "5px 8px", textAlign: "right", borderBottom: `0.5px solid ${T.border}`, color: T.text, fontWeight: 500 }}>{fmt(m.income)}</td>
...
export default function App() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const r = await window.storage?.get("jbk_vault_v1");
  ...
  // Persist vault
  useEffect(() => {
    if (!scenario) return; // don't persist on first render
    const save = async () => {
      try { await window.storage?.set("jbk_vault_v1", JSON.stringify(vault)); } catch {}
    };
    save();
  }, [vault]);
  ...
  const computed = useMemo(() => {
    ...
    if (wifeGross > 0 && scenario.babyPlanned && wifeCosts > wifeGross * 0.70) warnings.push("doubleTrap");
  ...
  const saveToVault = (slotId) => {
    if (!scenario) return;
    setVault(v => ({
      ...v,
      slots: {
        ...v.slots,
        [slotId]: { ...v.slots[slotId], scenario: { ...scenario }, engineResult: computed ? { cashflow: computed.cashflow, verdict: computed.verdict } : null, savedAt: Date.now() },
      },
    }));
  };

  const loadFromVault = (slotId) => {
  ...
  const clearVaultSlot = (slotId) => {
    setVault(v => ({ ...v, slots: { ...v.slots, [slotId]: { ...v.slots[slotId], scenario: null, engineResult: null, savedAt: null } } }));
  };
  ...
  const navSections = [
    ...
    { id: "vault",       label: lang === "id" ? "Skenario"   : "Scenarios",   disabled: !scenario },
  ...
  return (
    <div style={{ fontFamily: T.fontSans, maxWidth: 860, margin: "0 auto", padding: "16px 4px 40px", color: T.text }}>
      ...
      {/* Preset strip + year selector (when past screening) */}
      ...
      {section === "calculator" && scenario && (
        <CalculatorSection scenario={scenario} onUpdate={updateScenario} goals={goals} year={year} lang={lang} computed={computed} />
      )}
      ...
      {section === "plan" && (
        <PlanSection
          ...
          goals={goals} lang={lang}
        />
      )}

      {section === "vault" && (
        <VaultSection vault={vault} onSave={saveToVault} onLoad={loadFromVault} onClear={clearVaultSlot} scenario={scenario} computed={computed} lang={lang} />
      )}
...
// Named re-export for ChipItem used in ScreeningSection
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      ...
      style={{
        ...
        color: selected ? T.green800 : T.textMuted, fontWeight: selected ? 500 : 400,
        transition: "all .15s", opacity: disabled ? 0.4 : 1, fontFamily: T.fontSans,
      }}>
      {label}
    </button>
  );
